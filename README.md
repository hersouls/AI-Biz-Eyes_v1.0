# AI Biz Eyes 공모사업 자동화 관리 웹서비스 - Mock API 서버

AI Biz Eyes 공모사업 자동화 관리 웹서비스의 Mock API 서버입니다. API 명세서에 따라 구현된 모든 엔드포인트를 제공하며, 실제 데이터베이스 없이도 프론트엔드 개발 및 테스트가 가능합니다.
...
### 공고 목록 조회
```bash
curl -X GET "https://bizeyes.moonwave.kr/api/bids?page=1&limit=10" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

### 레퍼런스 등록
```bash
curl -X POST https://bizeyes.moonwave.kr/api/references \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{
